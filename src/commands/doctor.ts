...
import { getRepoPath, getSkillsyncDir } from "../core/config.js";
...
export function registerDoctor(program: Command): void {
  program
    ...
    .action(async (opts) => {
      ...
      print(
        `skillsync dir: ${getSkillsyncDir()} (${existsSync(getSkillsyncDir()) ? "ok" : "missing — run init"})`,
      );
      print(
        `skills repo: ${getRepoPath()} (${repoExists ? "ok" : "missing — run init"})\n`,
      );
